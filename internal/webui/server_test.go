...
func TestNewHTTPServerSetsHeaderTimeoutOnly(t *testing.T) {
	s := &Server{addr: ":0"}
	srv := s.newHTTPServer()
	...
	if srv.ReadTimeout != 0 {
		t.Errorf("ReadTimeout = %v, want 0: it would cut off large uploads and cancel long handlers", srv.ReadTimeout)
	}
