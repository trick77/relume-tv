...
- **`-entertainment-median-window`** &nbsp;·&nbsp; default `1` (off) — Entertainment mode: gated median spike filter on the DTLS send path — the number of recent TV frames the per-channel median is taken over. It **drops crass, single-frame flicker spikes** the TV emits, while passing normal content through verbatim with no trailing latency (the complement to `-entertainment-smooth-tau`, and composable with it — run the median alone by also setting the tau to `0`). `3` catches single-frame spikes, `5` catches two-frame ones. The "Jitter-reduction" dashboard card shows how many spikes it dropped per window.
- **`-entertainment-median-threshold`** &nbsp;·&nbsp; default `8000` — Entertainment mode: the per-component "crass" magnitude (of `65535`) above which the median filter may suppress a change; smaller changes always pass through untouched. Only active when `-entertainment-median-window` is greater than `1`. Tune it against the `bri_max_jump` / `col_max_jump` values in the `-debug` stream stats.
